...
class ICPCBalloons {
	...
	string now;
public:
	int minRepaintings(vector <int> bac, string bs, vector <int> ac) {
		static string now(bs.size(),'0');
		int sum_ac=accumulate(ac.begin(),ac.end(),0),
			sum_bac=accumulate(bac.begin(),bac.end(),0);
		if(sum_bac<sum_ac) return -1;
		if(sum_ac<=0) return 0;
		//-1 if size mismatch
		int min=0;
		for(int i=0;i<ac.size();i++){
			if(ac[i]<=0) continue;
			for(int j=0;j<bac.size();j++){
				if(now[i]!='0'&&now[i]!=bs[j]) continue;
				int temp1=ac[i],temp2=bac[j];
				if(ac[i]<=bac[j]) {bac[j]-=ac[i]; ac[i]=0;}
				else{ac[i]-=bac[j];bac[j]=0;}
				now[i]=bs[j];
				if(j==bac.size()-1 && ac[i]>0) return -1;
				int t=minRepaintings(bac,bs,ac)+ac[i];
				if(t<min)min=t;
				bac[j]=temp2;ac[i]=temp1; 
				if(ac[i]==0) now[i]='0';
			}
		}
		return min;
